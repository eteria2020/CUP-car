            ...
            if (sanitizeInput(req, res)) {
                pg.connect(conString, function(err, client, done) {
                    ...
                    if (typeof req.params.plate !== 'undefined' && req.params.plate != '') {
                        ...
                        client.query(
                            "SELECT EXISTS(SELECT plate FROM cars WHERE plate=$1)", [req.params.plate],
                            function(err, result) {
                                done();
                                if (result.rows[0].exists) {
                                    client.query(
                                        "SELECT EXISTS(SELECT car_plate FROM reservations WHERE (customer_id=$1 OR car_plate=$2) AND active IS TRUE)", [req.user.id, req.params.plate],
                                        function(err, result) {
                                            done();
                                            logError(err, err);
                                            if (result.rows[0].exists) {
                                                ...
                                                client.query(
                                                    "INSERT INTO reservations (ts,car_plate,customer_id,beginning_ts,active,length,to_send) VALUES (NOW(),$1,$2,NOW(),true,30,true) RETURNING id", [req.params.plate, req.user.id],
                                                    function(err, result) {
                                                        done();
                                                        logError(err, err);
