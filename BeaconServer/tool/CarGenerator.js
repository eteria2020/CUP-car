...
var hostNodeProd = "https://api.sharengo.it:8023";
...
Car.prototype.getCars = function getWitelist(){
    ...
    var time= microtime();
    var msg = this.plate+ ' : getCars  ' + (time-this.start);
    //console.log(msg);
    axios.get(hostNodeProd + '/v3/cars', {

    })
        .then(response => {
            //console.log(response.data);
            console.log("getCars " +countPoi++);
        })
        .catch(error => {
            console.log(error);
        });
...
Car.prototype.run = function() {
    ...
    setTimeout(()=>setInterval(this.getCars.bind(this),          1*10*1000),random(0,1*10*1000)); //impatto medio, sinceramente pensavo peggo, dopo un po' è arrivato a 4 di load comunque continua a salire
